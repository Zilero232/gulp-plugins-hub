  ...
  return GulpPluginFactory({
    ...
    onFile: async (file: FileVinyl) => {
      try {
        ...
        if (isBuffer) {
          ...
          content = await htmlMinifier.minify(content, minifyOptions);
        ...
        if (logStream) {
          GulpWinstonError({
            pluginName: PLUGIN_NAME,
            message: `Transformed HTML file ${file.relative} ${isBuffer ? "successfully." : "failed."}.`,
            options: {
              level: "info",
            },
          });
    ...
    onFinish: () => {
      if (logEnd) {
        GulpWinstonError({
          pluginName: PLUGIN_NAME,
          message: `Successfully processed. Total files: ${fileCount}, Total processed files: ${processedCount}, Total errors: ${errorCount} HTML file(s).`,
          options: {
            level: "info",
          },
        });
